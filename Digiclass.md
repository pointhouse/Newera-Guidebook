...
- 学生用户既不能查看具体的评分权重，也不能查看目前的实时分数，以便让学生尽可能聚焦精力在具体的课程和任务上。我们认为形成性评估应该是隐形的。

- 教师用户可在「班级管理」- 「成绩汇总」（网页端为「成绩详情」）中查看每个学生具体的得分情况。
...
- 从班级内点进对应教材（也可从「学习详情」点击模块进入），可随时查看单个学生的学习情况。方便老师适时进行教学干预。也可就教材内某个具体内容发起班级讨论。
...
- 也可对整个班级的学习进度形成顶层把握。可在网页端导出为多种 excel 表单。

- 支持「刷题检测」，系统会自动记录异常答题情况，并以「刷题率」呈现在「成绩详情」中。目前此数据仅限在网页端查看。

- 在移动端，「成绩汇总」会自动以直方图形式展示成绩分布情况。
...
!> 在网页端浏览题库时，若要查看单个题目详情，鼠标悬停即可展示；若要进入查看，建议右键“在新标签页/新窗口中打开”，以免重置浏览进度。
